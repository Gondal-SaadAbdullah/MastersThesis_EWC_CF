...
if __name__=="__main__":
  ...
  ax.text(250,0.5,"Initial training",size=25,bbox=dict(facecolor='yellow', alpha=1,boxstyle='round')) ;
  ax.text(1750,0.5,"Re-training",size=25,bbox=dict(facecolor='lightblue', alpha=1,boxstyle='round')) ;
  #ax.legend(fontsize=20,loc='lower left')
  ax.grid(True,which='both');
  x = np.arange(0,(D[1][:,0]).max()+50,1) ;
  ax.fill_between(x,0,1,where=(x>(x.shape[0]/2)),facecolor='gray',alpha=0.3)
  plt.tight_layout()
  figName = sys.argv[1]+".svg" ;
  plt.savefig("fig.svg") ;
  plt.savefig(figName) ;
